...
namespace FIX
{
  ...
  DEFINE_FLOAT(LegDV01);
  DEFINE_INT(SpreadType);
  DEFINE_FLOAT(LegNotionalNeutral);
  DEFINE_INT(PriceScalingFactor);
  DEFINE_INT(VolumeScalingFactor);
  DEFINE_STRING(MonitorRequestID);
  DEFINE_INT(NoMonitorIDs);
  DEFINE_INT(MonitorID);
  ...
  DEFINE_INT(MonitorRequestStatus);
  DEFINE_BOOLEAN(ClearPageIndicator);
  DEFINE_INT(MonitorPageNumRows);
  DEFINE_INT(MonitorPageNumColumns);
  DEFINE_INT(RowSpanStartColumn);
  DEFINE_INT(RowSpanLength);
  DEFINE_STRING(RowSpanText);
  DEFINE_INT(RowSpanAttribute);
  DEFINE_INT(MDPriceVolumeLevel);
}
#endif //FIX_FIELDS_H
